 ...
 */
app.controller('context1', ['$scope', '$http', '$timeout', '$filter', function ($scope, $http, $timeout, $filter) {
    //  初始数据
    ...
    $scope.getParams = function () {
        ...
        console.log($scope.getSplits('#param_agency'));
    ...
    $scope.selectTab = function ($event) {
        ...
        if (!tabFlag) {
            ...
            //  单选
            couple.find('label.ls').addClass('on');
            couple.find('label.rs').removeClass('on');
            tabOpt.attr('badg', '');
            $scope.flag = false;
    ...
    //  指标数据
    $http.get('../common/param_index.json').success(function (data) {
        if (data.errorNo == "0") {
            $scope.tabIndexs = data.tabIndex[0];
            console.log($scope.tabIndexs);
        }
    });
